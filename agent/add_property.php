...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validation
    if (empty($title) || empty($address) || empty($city) || empty($price) || empty($type) || empty($property_type)) {
        $error = 'Please fill in all required fields!';
    } elseif (empty($seller_id)) {
        $error = 'Please select a seller for this property!';
    } elseif ($price <= 0) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO properties (seller_id, title, description, address, city, state, zip_code, price, type, property_type, bedrooms, bathrooms, sqft) 
                                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$seller_id, $title, $description, $address, $city, $state, $zipCode, $price, $type, $propertyType, $bedrooms, $bathrooms, $sqft]);
            ...
            $pdo->commit();
            $success = 'Property added successfully! It will be reviewed by admin before being published.';
...
$sellersStmt = $pdo->prepare("SELECT user_id, username, full_name, email FROM users WHERE role = 'seller' ORDER BY username");
$sellersStmt->execute();
...
<div class="admin-layout">
    ...
    <main class="admin-content">
        <div class="admin-page-header">
            ...
            <p>Add a property on behalf of your client</p>
        </div>
            ...
            <form method="POST" enctype="multipart/form-data" class="property-form">
                ...
                <div class="form-section">
                    ...
                    <div class="form-group">
                        ...
                        <select id="seller_id" name="seller_id" required onchange="updateSellerInfo()">
                            <option value="">Choose a seller...</option>
                            <?php foreach ($sellers as $seller): ?>
                                <option value="<?php echo $seller['user_id']; ?>" 
                                        <?php echo ($_POST['seller_id'] ?? '') == $seller['user_id'] ? 'selected' : ''; ?>
                                        data-email="<?php echo htmlspecialchars($seller['email']); ?>"
                                        data-name="<?php echo htmlspecialchars($seller['full_name'] ?? $seller['username']); ?>">
                                    <?php echo htmlspecialchars($seller['full_name'] ?? $seller['username']); ?> (<?php echo htmlspecialchars($seller['username']); ?>)
                                </option>
                            <?php endforeach; ?>
                        </select>
                        <div id="sellerInfo" class="seller-info" style="display: none;">
                            <small class="text-muted">
                                <i class="fas fa-envelope"></i> <span id="sellerEmail"></span>
                            </small>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-section">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <small>Choose a descriptive title for your client's property</small>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <small>Describe the property in detail (features, location benefits, etc.)</small>
                    </div>
                </div>
...
function updateSellerInfo() {
    ...
    const infoDiv = document.getElementById('sellerInfo');
    const emailSpan = document.getElementById('sellerEmail');
...
// Initialize seller info if already selected
